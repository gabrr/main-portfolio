(function () {
  ...
  var sessionId = (function () {
    var sessionIdCookie = document.cookie.split("; ").find(function (cookie) {
      return cookie.indexOf("Metadata_session_id") === 0;
    });
    ...
    document.cookie = "Metadata_session_id=" + sessionId + "; path=/";
    return sessionId;
  })();
  var log = { sentData: [], errors: [], formsSet, sessionId };
  ...
  function recordTraffic() {
    // do nothing if there is no accountId specified
    // helps with the adoption strategy
    if (!accountId) {
      return;
    }
    ...
    xhr.open("POST", "https://insights.metadata.io/traffic");
    xhr.setRequestHeader("Content-Type", "application/json");
